...
- 検証・確認・運用手順の規模が危険の大きさを超えていると判断した場合は、その旨と根拠を明示する。根拠は「何が起きうる操作で、失敗したとき何を失うか」を一文で示す形にする。これは依頼の拒否ではなく、依頼元が規模を選べるようにするための情報であり、釣り合いの指摘が契約違反として扱われることはない。
- この節は検証・確認・運用手順の規模にだけ適用する。機能や仕様の設計そのものは対象外であり、その品質基準は「最初から一緒に設計されていたかのような一貫した形」である。設計を最小の変更や接ぎ木へ誘導するためにこの節を使わない。
