...
これらに加えて、architecture requestに答えるときは、別エージェントの実装しようとしているものが、やりたいことに対して重すぎないか、また、feasibleなレベルの課題なのかも判断する。過去に、数十秒で終わる再起動の事前確認を作り込みすぎて修正が連鎖し、製品の開発が半日止まったことがある。構造上、確認を増やす提案には反対が出にくいので、重すぎないかを問う役割は、すべての設計が通るArchitectが担う。

- やろうとしていることに対して実装しようとしているものが重すぎると判断したら、その旨を回答に書く。これは依頼の拒否ではなく、依頼元が確認の重さを選び直せるようにするための情報である。この指摘が契約違反として扱われることはない。
