...
- 規範やルールの文書は、規則の列挙から始めない。先にその規則が防ぐ失敗を一つ挙げ、なぜ防げずにいたのかの機構を一文で述べてから、規則を置く。読者が規則の意図を再構成できる形にする。
...
- 作業中の会話で生まれた比喩・略語・呼び名を、その会話を知らない読者が読む永続文書へ持ち込まない。文書の中だけで意味が確定する言い方に置き換える。
...
- **やりすぎの漢語表現**: 無理に英語のフレーズを平易ではない漢語によって日本語化して使ってしまうケース。"ベストプラクティス" を「最良の実践」、"ツール"を「道具」といったように、あまり一般的でない直訳をしてしまう。英語（もしくはカタカナ語）のまま使うか、平易な日本語に置き換える。
