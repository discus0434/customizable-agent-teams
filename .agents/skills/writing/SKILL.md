...
- 論証は一方向に進める。結論を出してから反論を処理し、結論を言い直す構成にしない。反論と疑念の処理を終えてから、結論を一度だけ述べる。
...
- 何かを否定・限定するときは、否定する命題そのものを「」で囲んで正確に書き出す（「明文化されていればすべてを任せられる」を意味しない、など）。「何もかもが解決するわけではない」のような漠然とした否定で済ませない。
- 「後の章で扱う」のような前方参照は、論証が一段落した位置（段落末・節末）に置く。論証の途中に挟んで流れを切らない。
- 規範やルールの文書は、規則の列挙から始めない。先にその規則が防ぐ失敗を一つ挙げ、なぜ防げずにいたのかの機構を一文で述べてから、規則を示す。読者が規則の意図を再構成できる形にする。
...
- 章・節をまたいで、同じ概念の扱いを一致させる。ある節で「人間が決める」と分類したものを、別の節で「チームで合意する」と書かない。分類・定義・用語の位置づけは全体で揃える。
...
- 譲歩や限定（「ただし」「とはいえ」）を挟んだら、その後で必ず論を進める。逆接で終えて宙吊りにしない。
...
- 後で参照する必要のない名前（ファイル名、関数名、識別子）を出さない。「仕様書」「金額計算のユーティリティ」のような一般的な言い方で済ませる。
- 抽象的な言い回しの指す内容が文脈から一意に決まらないときは、できるだけ丸括弧を添えてその場で特定し、読者に前を読み返させない。
...
書き上げたら語彙と言い回しを見直す。口頭の説明でも使うかどうかが目安になる。

- 英語の語やフレーズを、漢語へ一対一で置き換えない。英語に戻すと一語か一句になり、かつ日本語の会話では使わない言い方は、直訳を疑う。節の末尾に対応表を挙げる。
- 日本語に定着したカタカナ語は、そのまま使う。「ベストプラクティス」「ガードレール」「ツール」「トレードオフ」を和語や漢語へ訳し直さない。定着した術語（「冪等」「不変条件」「排他制御」）も、英語由来でよい。平易にするのは、耳慣れない直訳の側だけである。
- 「〜で書く」という形は不自然に響くので、「書く」を別の言い方に変える。「動詞で書く」「役割名で書く」「原綴りで書く」は、「文末を動詞にする」「役割名を使う」「アルファベットのまま示す」とする。「〜を使って書く」に替えても直らない。
- 文章の要素を、何でも「置く」で済ませない。「結論を置く」「規則を置く」「一文を置く」は、「結論を述べる」「規則を示す」「一文を添える」のように、その動作に合う動詞を選ぶ。場所とともに使う本来の形（「段落末に置く」「説明の代わりに置く」）は、そのままでよい。
- 動詞と目的語の組み合わせが自然かを、読み直しで確かめる。係り方が誤読を生む組み合わせ（「確認が守る危険」）は、防ぐ・招くのような向きの明確な動詞で言い直す（「その確認が防ぐ失敗」）。
- 動詞で言えることを、名詞と汎用動詞の組に膨らませない。悪い例：「検証の実施を行う」「品質の向上を図る」「合意の形成がなされる」。良い例：「検証する」「品質を上げる」「合意する」。
- 対象を指す語は具体的に選ぶ。「AI」「ツール」のような広い語でぼかさない。
- 抽象名詞で文を締めない（「〜という営み」「〜のありよう」「〜の総体」「〜という構造」）。文末を動詞にして、何がどうなるのかを示す。
- 主語と目的語の両方を抽象名詞にしない。どちらか一方は、人・もの・操作のような具体物にする。悪い例：「設計の意図が実装の細部へ浸透する」。良い例：「設計で決めたことを、実装する人が細部まで守る」。
- 比較や過不足を述べるときは、同じ種類の量どうしを比べる形にする。種類の違うものを比べる言い回し（「手順の規模が危険の大きさを超える」）は、どちらかの種類に揃えて言い直す（「手順が、防ぐ失敗に対して重すぎる」）。
- 比喩は、それが指す事実を同じ文か直後に書ける場合に限って使う。比喩だけで説明を終えない。指す内容が一意に決まらない比喩（「報告の外側に世界が広がっている」）や、慣用表現をひねった言い回し（「知識を体に入れる」）は使わない。平易な動詞でそのまま言う（「身につく」「気付く機会が減る」）。
- 術語の響きを持つ語を、術語でない場面に流用しない（システムから人間までの連なりを「経路」と呼ぶ、など）。「届くまでの流れ」「あいだに何があるか」のように、普通の言い方にする。
- 術語・訳語は、その分野で慣用されている語を選ぶ（プッシュ通知は「配送」ではなく「配信」、など）。意味の近い漢語を一般語の感覚で充てない。また、内部変数のように、意味が相手に伝わらない内輪語は使用しない。
- 作業中の会話で生まれた比喩・略語・呼び名を、あとに残る文書へ持ち込まない。その会話を知らない人が読むので、文書だけで意味が分かる言い方に置き換える。
...
- 場面を描写した直後に、その内容を要約し直さない。意味づけの一文（「このような作業は、ほぼ完全に任せられる」など）だけを添える。
...
- 接続や評価のためだけの文（「それ自体はよいことである」など）を入れない。
...
- 「本書もそれを否定しない」のような、著者の立場の弁明や断りを書かない。事実の記述（「〜に書かせる場合が多い」）だけにする。
...
- なぜその例を挙げるのか、なぜその結論に至るのか、読者が納得できるように説明する。相手の暗黙の理解を想定せず、説明を省略しない。
- 例が作為的に見えうる場合、それを隠さない。読者の疑念を先回りして認め、現実に十分あり得ることの根拠を添える。
