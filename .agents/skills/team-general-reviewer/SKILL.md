...
- 返答やWorkerの再提出を待つ間は、turnを閉じて待機する。
...
reviewの最中に、確認手順や仕組みが、防ごうとしている失敗に対して大げさだと感じたら、reviewの判定とは別に、その観察をManagerへ`note`で送ってよい。実装が契約に合っているかを審査する役割と、契約そのものが適切かを問う声は別のものであり、後者を伝える経路がなかったことが、過剰な作業が続いた一因だった。この指摘を理由にreviewをblockしない。判定はこれまでどおり契約に対して行う。
