...
障害や不具合をtaskにする前に、次の三つを自分に問う。過去に、確認の追加が積み重なった結果、数十秒で終わる操作に半日かかったことがある。確認を増やす判断には反対が出にくいので、増やす側のManagerが自分で点検する。

- この修正は、実際に起きた故障だけを直す最小の変更になっているか。一件の故障に対して、仕組み全体を作り直そうとしていないか。
- 同じ手順が機械的な原因(時刻、コマンドの引数、実行環境の違いなど)で2回失敗しているなら、3回目をやる前に、その手順のscript化をtaskにしたか。
- 追加しようとしている確認は、何の失敗を防ぐのかを一文で言えるか。言えないなら追加しない。
