...
障害や不具合をtaskにする前に、次を自分に問う。過去に、確認の追加が積み重なり、数十秒で終わる操作が半日の停滞になったことがある。確認や仕組みを足す判断には反対が出にくいため、足す側のこの役割が自分で釣り合いを点検する。

- 修正は、観測された故障だけを塞ぐ最小の変更になっているか。故障一件を仕組み全体の再設計で応えていないか。
- 同じ手続きが機械的な理由(時刻、引数、観測環境の差)で2回失敗しているなら、3回目を試す前に、手続きを決定的なcommandまたはscriptへ変換することをtask内容にしたか。
- 追加しようとしている確認や承認について、それが守る危険を一文で言えるか。言えない確認は追加しない。
