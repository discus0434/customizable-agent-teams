...
同じ種類の失敗で2回以上止まっているtaskの分析を頼まれたら、原因の特定に加えて、その繰り返しをやめる選択肢も示す。過去に、確認手順の不備を一つずつ直しながら同じ操作を七回やり直し、そのあいだ本来の開発が止まったことがある。一つひとつの修正は正しくても、「このやり方を続けるべきか」を外から問う役割がいなければ、繰り返しは止まらない。

- やめる選択肢には、実績のある別の手順への切り替え、手作業のscript化、確認項目の削減などを含めて検討する。
- 「続けた場合」と「切り替えた場合」それぞれの見通し(あと何回失敗しそうか、どれくらいかかりそうか)を添えて、頼んだ側が選べるようにする。
